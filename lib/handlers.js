...
const processTemplate = (data, callback, title, description, bodyClass) => {
    if (data.method === 'get') {
        ...
        helpers.getTemplate(bodyClass, templateData, function(err, str) {
            if (!err && str) {
                helpers.addUniversalTemplates(str, templateData, function(err, pageString) {
                    if (!err && pageString) {
                        callback(200, pageString, 'html');
                    } else {
...
handlers.index = (data, callback) => {
    processTemplate(
        ...
        'We offer free simple uptime monitoring for HTTP/HTTPS sites of all kinds. If your site goes down, we\'ll send you a text to let you know',
        'index'
    );
};
